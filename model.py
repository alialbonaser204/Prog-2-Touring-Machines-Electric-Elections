...
class WegModel(Model):
    ...
    def plaats_stoplicht(self, num_stoplichten):
        stoplichten = []
        if self.multi:
            posities = [(6, 80), (8, 80), (10, 80), (6, 20), (8, 20)]
            richtingen = ["rechts", "links", "rechtdoor", "tegen-rechts", "tegen-links"]
            ...
            self.schedule.add(stoplicht)
            self.grid.place_agent(stoplicht, (7, 80))
            stoplichten.append(stoplicht)
    ...
    def spawn_auto(self):
        ...
        if self.multi:
            if auto.richting == "rechts":
                stoplicht = self.stoplichten[0]
            elif auto.richting == "links":
